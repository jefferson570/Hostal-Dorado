...
// Fecha de la Política de privacidad vigente (public-site/privacidad.html).
// Si cambias esa política, actualiza esta versión.
const POLITICA_VERSION = '2026-09-25';
const { hoy: today } = require('../utils/fechas');

/* Precio "desde" que ve el cliente web. Las pre-reservas son estadías de
   12 h, así que se usa la tarifa por noche: el precio de una habitación
   que se cobra por bloques de pocas horas (p. ej. S/30 × 3 h) no aplica y
   haría creer que 12 h cuestan eso. Si un tipo solo tuviera tarifa por
   horas, se usa esa como último recurso. */
const SQL_TARIFARIO = `
  SELECT tipo,
         COALESCE(MIN(CASE WHEN unidad = 'noche' THEN precio END), MIN(precio)) AS precio,
         COUNT(*) AS total
  FROM rooms GROUP BY tipo ORDER BY tipo`;
...
function availability(req, res) {
  ...
  const oferta = db.prepare(SQL_TARIFARIO)
    .all()
    .map(r => ({
      tipo: r.tipo,
      precio: r.precio,
...
function tarifario(req, res) {
  const habitaciones = db.prepare(SQL_TARIFARIO).all();
...
function createReservation(req, res) {
  ...
  const row = {
    ...
    estado: 'pendiente',
    created_at: nowISO(),
    consentimiento_at: nowISO(),
    politica_version: POLITICA_VERSION,
  };

  db.prepare(`
    INSERT INTO reservations (id, tipo_habitacion, cantidad, checkin, checkout, hora_ingreso, room_id, nombre, telefono, email, estado, created_at, consentimiento_at, politica_version)
    VALUES (@id, @tipo_habitacion, @cantidad, @checkin, @checkout, @hora_ingreso, @room_id, @nombre, @telefono, @email, @estado, @created_at, @consentimiento_at, @politica_version)
  `).run(row);
...
/**
 * GET /api/public/hostal
 * Datos de identificación del hostal (los mismos que salen impresos en
 * boletas y facturas). Las páginas legales los usan para indicar quién es
 * el responsable de los datos personales. Se editan en Configuración.
 */
function hostal(req, res) {
  const { nombre_comercial, razon_social, ruc, direccion, telefono, email } = getAjustes(db);
  res.json({ nombre_comercial, razon_social, ruc, direccion, telefono, email });
}

module.exports = { availability, tarifario, createReservation, hostal };