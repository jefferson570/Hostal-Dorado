...
6. Abre en tu navegador:
   - Sitio público (clientes): **http://localhost:3000**
   - Panel del personal: **http://localhost:3000/panel**
...
>
> Sitio público para clientes: **http://localhost:3000** · Panel del personal:
> **http://localhost:3000/panel** (el rol se detecta solo a partir del usuario).
>
> Páginas legales del sitio: `/privacidad`, `/cookies`, `/terminos`,
> `/seguridad` y `/.well-known/security.txt`. Los datos del responsable (razón
> social, RUC, correo) salen de **Configuración → Datos del hostal**.
> Las fotos reales van en `public-site/img/fotos/` (ver el `LEEME.txt` de esa carpeta).
...
- **Mensajes de error genéricos en login**: nunca se revela si el usuario
  existe o no, solo "usuario o contraseña incorrectos".
