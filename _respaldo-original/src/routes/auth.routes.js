...
router.patch('/me',
  ...
  [
    ...
    body('foto').optional({ values: 'falsy' }).isString().withMessage('Foto inválida.')
      .custom(v => !v || v.startsWith('data:image/')).withMessage('La foto debe ser una imagen válida.'),
    body('password_nueva').optional({ values: 'falsy' }).isLength({ min: 6 }).withMessage('La nueva contraseña debe tener al menos 6 caracteres.'),
