...
$$('.role-toggle button').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    ...
    $('#role-toggle').classList.toggle('is-recepcion', selectedRole === 'recepcion');
  });
});
...
const pageMeta = {
  ...
  caja:{title:'Caja y comprobantes', sub:'Cierre de caja por método de pago, boletas y facturas'},
  configuracion:{title:'Configuración', sub:'Tu cuenta, datos del hostal y usuarios del sistema'},
};
...
function closeModal(){
  ...
  const gen = modalGen;
  setTimeout(()=>{ if(gen !== modalGen) return; bd.classList.remove('open','closing'); $('#modal-box').innerHTML=''; }, 170);
