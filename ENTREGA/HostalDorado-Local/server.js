...
/* ---------- Frontends ---------- */
const staticOpts = { maxAge: config.isProd ? '1h' : 0 };
// Panel de gestión (requiere login con rol) — es el frontend principal en "/".
app.use(express.static(path.join(__dirname, 'public'), staticOpts));
// Sitio público de clientes — se sirve en /sitio.
app.use('/sitio', express.static(path.join(__dirname, 'public-site'), staticOpts));

// Cualquier otra ruta devuelve el HTML del frontend correspondiente.
app.get('*', (req, res) => {
...
server.listen(config.port, () => {
  ...
  console.log(`    Panel del personal: http://localhost:${config.port}/`);
  console.log(`    Sitio público:      http://localhost:${config.port}/sitio\n`);
});
